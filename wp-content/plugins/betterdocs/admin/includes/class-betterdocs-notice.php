...
//                break;
            case 'black_friday_22' :
                do_action( 'wpdeveloper_black_friday_22_notice_for_' . $this->plugin_name );
                $this->get_message( 'black_friday_22' );
                $this->dismiss_button_scripts();
                break;
            case 'review' :
    ...
    public function blackFridayNotice(){
        ...
        if ( array_key_exists( 'betterdocs-pro/betterdocs-pro.php', get_plugins() ) ) {
            return;
        }

		if ( get_option( 'bdocs_black_friday_notice', false ) ) {
			return;
		}

        $time     = current_time('timestamp');
        if( $time > 1669852799 ) {
            update_option( 'bdocs_black_friday_notice', true );
            return;
        }

        add_action('admin_notices', [$this, 'blackNotice']);
        add_action( 'wp_ajax_bdocs_black_friday_notice_dismiss', [ $this, 'black_friday_notice_dismiss' ] );
    }

    public function black_friday_notice_dismiss() {
		check_ajax_referer( 'bdcos_black_friday_nonce', 'security' );
		update_option( 'bdocs_black_friday_notice', true );
		wp_send_json_success();
	}

    public function blackNotice(){
        $ajax_url = admin_url( 'admin-ajax.php' );
		$nonce    = wp_create_nonce( 'bdcos_black_friday_nonce' );
        ?>
        <div class="bdocs-wpnotice-wrapper notice  notice-info is-dismissible bddocs-black-friday-notice">
            <div class="bdocs-wpnotice-content-wrapper" style="display: flex; align-items: center;">
                <?php echo  $this->get_thumbnail('review'); ?>
                <div class="bdocs-black-friday-notice wpdeveloper-notice-message">
                    <p><span class="party-icon">&#127881;</span> <?php _e( '<strong>Black Friday Exclusive:</strong> SAVE up to 40% & access to BetterDocs Pro features.', 'betterdocs' ); ?>
                    <a class="betterdocs-e-button" href="https://betterdocs.co/upgrade" target="_blank"><?php _e( 'Grab The Offer', 'betterdocs' ); ?></a>
                    </p>
                </div>
            </div>
        </div>
        <script>
            (function ($) {
                $(document).on('click', '.bddocs-black-friday-notice button.notice-dismiss', function (e) {
                    e.preventDefault();

                    var $notice_wrapper = $(this).closest('.bddocs-black-friday-notice');

                    $.ajax({
                        url: "<?php echo esc_html( $ajax_url ); ?>",
                        type: "POST",
                        data: {
                            action: "bdocs_black_friday_notice_dismiss",
